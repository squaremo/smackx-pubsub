 ...
 * 
 * <h1>An Item received from a node (via {@link Node#getItems()} or {@link Node#addItemEventListener(org.jivesoftware.smackx.pubsub.listener.ItemEventListener)}</b>
 ...
 * 
 * <h1>An Item created to send to a node (via {@link Node#send()} or {@link Node#publish()}</b>
 ...
 */
public class Item<E extends PacketExtension> implements PacketExtension
{
	private String id;
	private E payload;
	 ...
	 * set to false, no <tt>Item</tt> is sent to the node, you have to use {@link Node#send()} or {@link Node#publish()}
	 ...
	 */
	public Item(String itemId, E payloadExt)
	{
		id = itemId;
		payload = payloadExt;
	}
